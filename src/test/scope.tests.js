...
import {Scope} from "../main/calvin/scope";
...
describe("Scope tests", function () {
    ...
    it("scope idden properties", function () {
        let scope = new Scope();
        [
            '$id',
            '$parent',
    ...
    it("a scope can be created with a parent and initial properties", function () {
        let parent = new Scope();
        let scope = new Scope(parent, {x: true});
        assert.isTrue(scope.hasOwnProperty('x'));
        assert.isTrue(scope.x);
    });

    it("if parent is not a Scope then child is attached to Scope.$root", function () {
        let $root = new Scope();
        Scope.$root = $root;
        let scope = new Scope();
    ...
    it("a scope can move", function () {
        let parentA = new Scope();
        parentA.p = 'a';
        let parentB = new Scope();
        parentB.p = 'b';
    ...
    // it("scope $changed requires a callback", function () {
    //     expect(() => new Scope().$watch("anything")).to.throw(TypeError);
    // });

    it("scope $changed notifies callback until stopped", function () {

        let scope = new Scope({x: 0});

        assert.deepEqual(Object.keys(scope), ['x']);

        let count = 0;

        scope.$watch('x').changed(function first() {
            count++;
        });
        ...
        scope.$watch('x').changed(function second() {
            extra = true;
        });
    ...
    it("scope.$watch('...').changed notifies callback once if required", function () {
        ...
        let promise = scope.$watch('x').changed(once);
    ...
    it("$evalAsync simple test", async function () {
        ...
        let scope = new Scope({x: 0, y: undefined});
